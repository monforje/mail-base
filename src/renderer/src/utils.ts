...
export const detectFileType = (content: string): 'users' | 'packages' | 'invalid' => {
...
export const validateFileContent = (content: string, expectedType: 'users' | 'packages'): { isValid: boolean; errors: string[] } => {
  const lines = content.split('\n').filter(line => line.trim() !== '')
  const errors: string[] = []
  ...
  lines.forEach((line, index) => {
    ...
    if (expectedType === 'users') {
      if (!validateUserLine(line)) {
        const parts = line.split('\t')
        if (parts.length !== 3) {
          errors.push(`Строка ${lineNumber}: неверное количество полей (ожидается 3)`)
        } else {
          const [phone, fullName, address] = parts
          if (!validatePhoneNumber(phone)) {
            errors.push(`Строка ${lineNumber}: неверный формат телефона (ожидается 8XXXXXXXXXX)`)
          }
          if (!fullName.trim()) {
      ...
      if (!validatePackageLine(line)) {
        const parts = line.split('\t')
        if (parts.length !== 4) {
          errors.push(`Строка ${lineNumber}: неверное количество полей (ожидается 4)`)
        } else {
          const [senderPhone, receiverPhone, weight, date] = parts
          if (!validatePhoneNumber(senderPhone)) {
            errors.push(`Строка ${lineNumber}: неверный формат телефона отправителя (ожидается 8XXXXXXXXXX)`)
          }
          if (!validatePhoneNumber(receiverPhone)) {
            errors.push(`Строка ${lineNumber}: неверный формат телефона получателя (ожидается 8XXXXXXXXXX)`)
          }
          if (!validateWeight(weight)) {
            errors.push(`Строка ${lineNumber}: неверный формат веса (ожидается положительное число)`)
          }
          if (!validateDate(date)) {
            errors.push(`Строка ${lineNumber}: неверный формат даты (ожидается "dd mmm yyyy", например "15 jan 2025")`)
          }
        }
      }
    }
